...
int switch_bootstrap()
{
    ...
    if (!(d = opendir(SYSFS_CLASS_SWITCH_PATH))) {
        LOGE("Unable to open '%s' (%s)", SYSFS_CLASS_SWITCH_PATH,
                   strerror(errno));
        return -errno;
    }
    ...
    while ((de = readdir(d))) {
        ...
        if (disp_bootstrap_switch(tmp)) {
            LOGE("Error bootstrapping switch '%s' (%s)", tmp,
                      strerror(errno));
        }
    }
...
{
#if DEBUG_BOOTSTRAP
    LOG_DISP("bootstrap_switch(%s):", sysfs_path);
#endif
