...
AudioHeadsetConnected(void *arg, void * user_data)
{
    ...
    /* When connect is initiated during a device inquiry period
     * it results in a missing device inquiry completed event
     * and therefore we issue an explicit inquiry off
     * Was observed on FFA running Android Gingerbread */
...
static void *HCIFilterThread(void *arg)
{
    ...
    while (1) {
        ...
        if (pBuffer[0] == HCI_EVENT_PKT) {
            ...
            if(*packedOpCode == cmd_opcode_pack(OGF_LINK_CTL, OCF_INQUIRY_CANCEL))
            {
                A_DEBUG("Device Inquiry Completed\n");
                pAbfBtInfo->btInquiryState &= ~(1 << 0);
                if(!pAbfBtInfo->btInquiryState) {
