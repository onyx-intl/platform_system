...
ieee80211_find_conn(AR_SOFTC_DEV_T *arPriv, A_UINT8 *node_addr)
{
    ...
    A_PRINTF("AR6K: %s : %x:%x:%x:%x:%x:%x\n", __func__,   node_addr[0], node_addr[1], node_addr[2], node_addr[3], node_addr[4], node_addr[5]); 
        ...
        wmi_pmparams_cmd(arPriv->arWmi,
            ...
            if (ar->arVersion.targetconf_ver == AR6003_SUBVER_DEFAULT) {
                ...
                if (arPriv->arNetworkType != AP_NETWORK) {
                    ...
                    A_PRINTF("AR6K: bg scan interval = %d, active dwell time = %d passive dwell time = %d\n",
